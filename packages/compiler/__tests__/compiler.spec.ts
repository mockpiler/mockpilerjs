import { validContext, validCode, customContextAccessor } from './test-cases'
import { mock, raw, mergeMockContexts, transformers } from '../src'

describe('compiler', () => {
  test('should compile valid code', () => {
    expect(mock(validContext)`${raw(validCode)}`).toMatchSnapshot()
  })
  ...
  test('should compile with merged contexts', () => {
    const compile = mock(mergeMockContexts(validContext, customContextAccessor))
